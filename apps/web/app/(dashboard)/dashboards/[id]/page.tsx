...
export default function DashboardBuilderPage() {
  ...
  const widgetTypes = [
    { value: 'chart', label: t.dashboards.chart, icon: BarChart3, description: t.dashboards.chartDesc },
    { value: 'table', label: t.dashboards.table, icon: TableIcon, description: t.dashboards.tableDesc },
    { value: 'kpi', label: t.dashboards.kpiCard, icon: TrendingUp, description: t.dashboards.kpiCardDesc },
    { value: 'text', label: t.dashboards.text, icon: Type, description: t.dashboards.textDesc },
  ] as const;

  const chartTypes = [
    { value: 'bar', label: t.dashboards.barChart, icon: BarChart3 },
    { value: 'line', label: t.dashboards.lineChart, icon: LineChart },
    { value: 'pie', label: t.dashboards.pieChart, icon: PieChart },
    { value: 'scatter', label: t.dashboards.scatterPlot, icon: BarChart3 },
  ];
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-auto p-6">
        {activeDashboard.widgets.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-zinc-400 font-medium">{t.dashboards.noWidgets}</p>
            <p className="text-zinc-500 text-sm mt-1">
              {t.dashboards.switchToEdit}
            </p>
            {!editMode && (
              <Button
                  ...
                  {widget.type === 'table' && (
                    <div className="h-full overflow-auto text-sm text-zinc-300">
                      {(widget.config.rows as unknown[][]) ? (
                        ...
                        <p className="text-zinc-500 text-center mt-8">{t.dashboards.noDataConfigured}</p>
                      )}
                    </div>
                  )}
                  {widget.type === 'text' && (
                    <div className="p-2 text-sm text-zinc-300 prose prose-invert prose-sm max-w-none">
                      {(widget.config.content as string) || t.dashboards.noContent}
                    </div>
                  )}
                </div>
              </div>
      ...
      <Dialog open={showAddWidget} onOpenChange={setShowAddWidget}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white">{t.dashboards.addWidget}</DialogTitle>
            <DialogDescription className="text-zinc-400">
              {t.dashboards.chartDesc}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-1.5">
                {t.dashboards.dataSource}
              </label>
              <Select value={newWidgetFileId} onValueChange={setNewWidgetFileId}>
                <SelectTrigger className="bg-zinc-800 border-zinc-700 text-white">
                  <SelectValue placeholder={t.dashboards.selectFile} />
                </SelectTrigger>
                <SelectContent className="bg-zinc-900 border-zinc-800">
                  <SelectItem value="none">{t.dashboards.noFile}</SelectItem>
                  {files.map((f) => (
